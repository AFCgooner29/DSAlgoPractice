...
Last updated: Apr 19 2020, 21:34
...
public class Expogo {
	public static class pair{
		...
		long y;
		String Dir;
		public pair(long a,long b,String s1) {
			x = a;
			y = b;
			Dir = s1;
		}
	...
	public static String BFS(int x,int y) {
		...
		while(!qu.isEmpty()) {
			...
			if(curr.x == x && curr.y ==y) {
			  ...
			  i = curr.Dir.length()+1;
			qu.add(new pair(curr.x, curr.y-(long)Math.pow(2, i-1), curr.Dir+"S"));
			qu.add(new pair(curr.x+(long)Math.pow(2, i-1), curr.y, curr.Dir+"E"));
			qu.add(new pair(curr.x, curr.y+(long)Math.pow(2, i-1), curr.Dir+"N"));
			qu.add(new pair(curr.x-(long)Math.pow(2, i-1), curr.y, curr.Dir+"W"));
		}
		return "IMPOSSIBLE";
	}
