...
Last updated: Apr 19 2020, 23:00
...
public class Join_the_ranks {
	...
	public static void finder(Stack<pair> st){
		Stack<pair> A = new Stack<pair>();
		Stack<pair> B = new Stack<pair>();
		int Aidx = 0;
		A.push(st.pop());
 		for (int i = 1; i < st.size(); i++) {
			if(st.get(i).y == st.get(i-1).y) {
				Aidx++;
				A.push(st.pop());
			}
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <=tot; i++) {
			int r = sf.nextInt();
			int s = sf.nextInt();
			Stack<pair> test = new Stack<pair>();
			for (int j = r; j >0; j--) {
				for (int j2 = s; j2 >0; j2--) {
					test.push(new pair(j2,j));
				}
			}
		}
		sf.close();
